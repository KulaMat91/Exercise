...
public class Task4 {
    public static class BankAccount {
        ...
        public BankAccount(Integer amount) {
            this.amount = amount;
            System.out.printf("Initialaid bank account with amount %d", amount);
        }

        public synchronized void deposit(Integer deposit) {
            amount += deposit;
            System.out.printf("%s New deposit: %d. Current amount: %d\n", Thread.currentThread().getName(), deposit, amount);
            notify();
        }
    ...
    public static void exercise4() {
        ...
        WithdrawThread threadD = new WithdrawThread(3000, account, "Thread E");
        DepositThread threadE = new DepositThread(10000, account, "Thread D");
